...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (result.requiresMfa) setStep("mfa");
  ...
  useEffect(() => { if (mfaCode.every((d) => d !== "")) handleMfa(); }, [mfaCode]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-950 via-primary-900 to-slate-900 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-primary-300 text-sm mt-1">Pharmacy Portal</p>
        </div>
          ...
          {step === "credentials" ? (
            <>
              <div className="mb-6">
                ...
                <p className="text-sm text-text-secondary mt-1">Sign in to manage dispensing</p>
              </div>
              <form onSubmit={handleLogin} className="space-y-4">
                <Input label="Email address" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="pharmacist@pharmacy.com" required autoFocus />
                ...
                <Button type="submit" loading={loading} className="w-full" size="lg" icon={<ArrowRight className="h-4 w-4" />}>Sign in</Button>
              </form>
              <div className="mt-6 pt-6 border-t border-border">
                <p className="text-xs text-text-tertiary text-center">Protected by QES Flow. All sessions are monitored and audited.</p>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="flex justify-center gap-2 mb-6">
                {mfaCode.map((digit, i) => (
                  <input key={i} ref={(el) => { mfaRefs.current[i] = el; }} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={(e) => handleMfaInput(i, e.target.value)} onKeyDown={(e) => handleMfaKeyDown(i, e)} className="w-11 h-13 text-center text-xl font-semibold rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors" autoFocus={i === 0} />
                ))}
              </div>
              {error && (
              ...
              <Button loading={loading} onClick={handleMfa} className="w-full" size="lg">Verify</Button>
              <button onClick={() => { setStep("credentials"); setError(null); }} className="mt-4 w-full text-sm text-text-secondary hover:text-text-primary text-center cursor-pointer">Back to sign in</button>
            </>
          )}
        </div>
      </div>
