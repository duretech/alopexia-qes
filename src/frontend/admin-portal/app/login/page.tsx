      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center h-14 w-14 rounded-2xl bg-primary-500/20 backdrop-blur-sm border border-primary-400/30 mb-4">
            <ShieldAlert className="h-7 w-7 text-primary-300" />
          </div>
          ...
          {step === "credentials" ? (
            <>
              <div className="mb-6">
                ...
                <p className="text-sm text-text-secondary mt-1">Sign in to the compliance dashboard</p>
              </div>
              <form onSubmit={handleLogin} className="space-y-4">
                <Input label="Email address" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@company.com" required autoFocus />
              ...
              <div className="mt-6 pt-6 border-t border-border">
                <p className="text-xs text-text-tertiary text-center">Admin sessions require MFA. All actions are logged.</p>
              </div>
            </>
          ) : (
            <>
