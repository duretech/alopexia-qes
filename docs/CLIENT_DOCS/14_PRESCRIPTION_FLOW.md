...
├─ Non-repudiation: Clinic cannot deny signing
├─ Proof: Signature proves clinic authorized
...
├─ Call SMS provider (GatewayAPI)
├─ SMS provider sends SMS
└─ SMS provider returns success

Result:
├─ SMS arrives: "Your QES Flow OTP: 123456 (expires in 10 min)"
└─ Clinic sees: "OTP sent, check your phone"
```
...
Clinic Enters:
├─ PIN: 1234 (only clinic knows this)
├─ Clicks "Verify"
...
├─ Clinic ID: (auto-filled from clinic profile)

Frontend Shows:
├─ File preview (clinic can see PDF)
├─ File information
...
└─ Included with request (invisible to clinic)
```
...
1. Verify JWT token:
   ...
   ├─ Extract user_id: clinic-123
   └─ Validate: User is a clinic ✅
...
4. Check permissions:
   ├─ Does clinic have PRESCRIPTION_UPLOAD permission? ✅
   └─ Is clinic in same tenant? ✅
```
...
├─ Clinic must upload new prescription
...
└─ If patient has bad reaction, contact clinic/poison control
```
